...
const ERROR_MAP: Record<number, UploadError> = {
...
export const useUploads = () => {
    const uploads = useState<UploadItem[]>('uploads', () => [])
    ...
    const updateUpload = (id: string, updates: Partial<Omit<UploadItem, 'id'>>) => {
        const index = uploads.value.findIndex((u: { id: string }) => u.id === id)
        if (index !== -1) {
            uploads.value[index] = { ...uploads.value[index], ...updates, id } as UploadItem
        }
    }
    ...
    const uploadFileSingleRequest = async (file: File, parentID: string | null = null) => {
        const id = addUpload(file)
        if (file.size > limit.value || usage.value + file.size > limit.value) {
            updateUpload(id, { status: 'error', error: 'upload.error.quotaExceeded' })
            return
        }
        ...
        url.searchParams.append('contentType', file.type || 'application/octet-stream')
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            xhr.upload.onprogress = (e) => {
                if (e.lengthComputable) {
                    ...
                    if (timeDiff > 500) {
                        ...
                        const current = uploads.value.find((u: { id: string }) => u.id === id)
                        if (current) {
            ...
            xhr.onload = () => {
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    const isExplorerPage = ['index', 'files-path'].includes(route.name as string)
            ...
            xhr.onerror = () => {
                updateUpload(id, { status: 'error', error: 'upload.error.networkError' })
    ...
    const uploadFileChunked = async (file: File, parentID: string | null = null) => {
        const id = addUpload(file)
        ...
        try {
            ...
            const chunkTasks = Array.from({ length: totalChunks }, (_, index) => {
                ...
                return async () => {
                    let retryCount = 0
                    let partInfo: { partNumber: number; etag: string; size: number } | null = null
                        ...
                        try {
                            partInfo = await uploadChunk(
                                ...
                                (loaded) => {
                                    ...
                                    const now = Date.now()
                                    if (now - lastSpeedTime > 200) {
                                        const totalLoaded = loadedByChunk.reduce((a, b) => a + b, 0)
                                        ...
                                        if (timeDiff > 0 && loadedDiff > 0) {
                                         ...
                                         const progress = Math.min(Math.round((totalLoaded * 100) / file.size), 99)
    ...
    const uploadFile = async (file: File, parentID: string | null = null) => {
        if (file.size >= CHUNKED_UPLOAD_THRESHOLD) {
            console.log(`Using parallel chunked upload for ${file.name} (${file.size} bytes)`)
            return uploadFileChunked(file, parentID)
        } else {
            console.log(`Using single-request upload for ${file.name} (${file.size} bytes)`)
