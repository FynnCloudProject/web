...
export const useAudioPlayer = () => {
  ...
  const play = (file: FileItem, contextFiles: FileItem[] = []) => {
    ...
    // If no context provided and playlist is empty or file not in playlist, make single item playlist
      ...
      !playlist.value.find((f) => f.id === file.id)
    ) {
      playlist.value = [file];
    }
  };
  ...
  const handleError = async (e: Event) => {
    // We can expose this to the audio element in AppAudioPlayer
    const audio = e.target as HTMLAudioElement;
    if (audio.error) {
      const { handle401 } = useTokenRefresh();
      const refreshed = await handle401();

      if (refreshed && currentTrack.value) {
        const currentTime = audio.currentTime;
        const originalSrc = trackSrc.value;

        // Force reload
        const separator = originalSrc.includes("?") ? "&" : "?";
        audio.src = `${originalSrc}${separator}t=${Date.now()}`;
