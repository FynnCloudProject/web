export const useDevConfig = () => {
  ...
  const networkDelay = import.meta.env.DEV
    ? useState<number>("dev-network-delay", () => 0)
    : ref(0);
  const isDevMenuOpen = import.meta.env.DEV
    ? useState<boolean>("dev-menu-open", () => false)
    : ref(false);

  const forceError = import.meta.env.DEV
    ? useState<boolean>("dev-force-error", () => false)
    : ref(false);
  const simulate401 = import.meta.env.DEV
    ? useState<boolean>("dev-simulate-401", () => false)
    : ref(false);
  const slowUploads = import.meta.env.DEV
